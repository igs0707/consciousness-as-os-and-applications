...
class DensityMA_OS_Full:
    ...
    Simulation 03: ds007537 (smartphone)
    Smartphone daily use (notifications and task switching)
    """
    def __init__(self):
        # OS Layer
    ...
    def generate_smartphone_stimulus(self, N=2000):
        """Generate stimulus pattern for ds007537: Smartphone daily use with frequent notifications and task switching"""
        np.random.seed(42)
        d_eff = np.zeros(N)
        
        # Frequent short bursts (notifications) + task switching
        for i in range(0, N, 45):          # notification every ~45 steps
            if i + 8 < N:
                # Sharp notification spike
                d_eff[i:i+8] = 0.88 + 0.12 * np.random.randn(8)
        
        # Longer task blocks with switching
        blocks = [
            (0, 300, 0.55),   # Email / work task
            (300, 550, 0.72), # Social media scroll
            (550, 800, 0.45), # Messaging switch
            (800, 1100, 0.85),# Video / media
            (1100, 1400, 0.38),# App switching
            (1400, 1700, 0.68),# Shopping / browsing
            (1700, 2000, 0.52) # Mixed final phase
        ]
        
        for start, end, base in blocks:
            length = end - start
            noise = 0.22 * np.random.randn(length)   # higher noise for task switching
            d_eff[start:end] = np.clip(d_eff[start:end] + base + noise, 0.15, 0.95)
        
        # Extra sharp transitions to maximize Identity Shake
        switch_points = [300, 550, 800, 1100, 1400]
        for p in switch_points:
            if p + 50 < N:
                d_eff[p-25:p+25] += 0.35 * np.random.randn(50)
        
        return np.clip(d_eff, 0.08, 0.98)
...
if __name__ == "__main__":
    print("=== Simulation 03: ds007537 (smartphone) ===")
    print("Smartphone daily use (notifications and task switching)")
    print("Testing robustness under highly fragmented, real-life digital stimuli\n")

    model = DensityMA_OS_Full()
    N = 2000
    d_eff = model.generate_smartphone_stimulus(N)
    ...
    for t in range(1, N):
        ...
        # Record nostalgia trace during calmer periods
        if (t % 120 < 40):
    ...
    fig.suptitle('Simulation 03: ds007537 (smartphone)\n'
                 'DensityMA_OS + Higher-Order Emotions (Notifications & Task Switching)', fontsize=14, fontweight='bold')

    axs[0].plot(d_eff, color='blue', label='D_eff (Smartphone notifications & task switching)')
    ...
    plt.tight_layout()
    plt.savefig('03_ds007537_smartphone_Simulation_Result.png', dpi=300, bbox_inches='tight')
    print("\n✅ Graph saved as '03_ds007537_smartphone_Simulation_Result.png'")
    print("Ready for next simulation (4/17).")