...
class DensityMA_OS_Full:
    ...
    Simulation 15: End-of-Life gradual decline
    Gradual physiological decline (hypoxia, low oxygen, terminal phase)
    """
    def __init__(self):
        # OS Layer
    ...
    def generate_end_of_life_gradual_stimulus(self, N=2000):
        """Generate stimulus pattern for End-of-Life gradual decline: Slow physiological decline"""
        np.random.seed(42)
        d_eff = np.zeros(N)
        
        # Gradual linear decline (simulating hypoxia / terminal fatigue)
        t = np.arange(N)
        decline = 0.62 - 0.00027 * t          # Slow drop from normal to near-critical
        noise = 0.055 * np.random.randn(N)
        d_eff = decline + noise
        
        # Occasional weak terminal surges (brief gamma-like bursts near end)
        surge_starts = [1250, 1480, 1720]
        for start in surge_starts:
            if start + 80 < N:
                surge = 0.28 * np.exp(-0.018 * np.arange(80))   # fading terminal surge
                d_eff[start:start+80] += surge
        
        # Very low density in the final phase
        return np.clip(d_eff, 0.08, 0.68)
    ...
    def step(self, d_eff, ma_prev, relation_ma_prev, nostalgia_trace, t,
        ...
        # Nostalgic repair (stronger in late phase as life review)
        nostalgia = 0.0
        boost = 2.0 if is_shaking and t > 1200 else 1.8 if is_shaking else 1.0
        ...
        # Zero-distance self-embrace (especially persistent in terminal phase)
        embrace_boost = 2.4 if is_shaking and t > 1000 else 1.6 if is_shaking else 1.0
...
if __name__ == "__main__":
    print("=== Simulation 15: End-of-Life gradual decline ===")
    print("Gradual physiological decline (hypoxia / terminal phase)")
    print("Testing self-embrace persistence when Q slowly approaches critical threshold\n")

    model = DensityMA_OS_Full()
    N = 2000
    d_eff = model.generate_end_of_life_gradual_stimulus(N)
    ...
    for t in range(1, N):
        ...
        # Record nostalgia trace (life review in terminal phase)
        if t > 900:
    ...
    fig.suptitle('Simulation 15: End-of-Life gradual decline\n'
                 'DensityMA_OS + Higher-Order Emotions (Gradual Physiological Decline)', fontsize=14, fontweight='bold')

    axs[0].plot(d_eff, color='blue', label='D_eff (Gradual decline)')
    ...
    plt.tight_layout()
    plt.savefig('15_End_of_Life_gradual_decline_Simulation_Result.png', dpi=300, bbox_inches='tight')
    print("\n✅ Graph saved as '15_End_of_Life_gradual_decline_Simulation_Result.png'")
    print("Ready for next simulation (16/17).")