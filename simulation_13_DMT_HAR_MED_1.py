...
class DensityMA_OS_Full:
    ...
    Simulation 13: DMT-HAR-MED (ds006644)
    DMT + harmine + meditation (ultra-powerful short-term dissolution + introspection)
    """
    def __init__(self):
        # OS Layer
    ...
    def generate_dmt_har_med_stimulus(self, N=2000):
        """Generate stimulus pattern for DMT-HAR-MED (ds006644): Ultra-powerful short dissolution + rapid introspection"""
        np.random.seed(42)
        d_eff = np.zeros(N)
        
        # Phase 1: Acute ultra-strong dissolution (very short, extremely intense)
        d_eff[0:280] = 0.92 + 0.35 * np.random.randn(280)
        # Sudden deep dissolution drops
        dissolution_points = [80, 140, 210]
        for p in dissolution_points:
            if p + 40 < N:
                d_eff[p:p+40] *= 0.35   # extreme ego dissolution
        
        # Phase 2: Transition (rapid recovery + introspection)
        d_eff[280:920] = 0.58 + 0.32 * np.random.randn(640)
        
        # Phase 3: Persistent re-integration & deepened state
        d_eff[920:2000] = 0.52 + 0.18 * np.random.randn(1080)
        
        # Add DMT-typical ultra-fast bursts
        burst_starts = [45, 95, 165, 245, 680, 740]
        for start in burst_starts:
            if start + 35 < N:
                burst = 0.68 * np.sin(2 * np.pi * np.arange(35) / 7)
                d_eff[start:start+35] += burst
        
        return np.clip(d_eff, 0.10, 0.98)
    ...
    def step(self, d_eff, ma_prev, relation_ma_prev, nostalgia_trace, t,
        ...
        # Nostalgic repair (stronger during transition and persistent phases)
        nostalgia = 0.0
        boost = 2.2 if is_shaking and t > 400 else 1.8 if is_shaking else 1.0
        ...
        # Zero-distance self-embrace (extremely strong during acute dissolution)
        embrace_boost = 2.8 if is_shaking and t < 500 else 1.8 if is_shaking else 1.0
...
if __name__ == "__main__":
    print("=== Simulation 13: DMT-HAR-MED (ds006644) ===")
    print("DMT + harmine + meditation (ultra-powerful short-term dissolution + introspection)")
    print("Testing strongest dissolution and fastest self-embrace recovery\n")

    model = DensityMA_OS_Full()
    N = 2000
    d_eff = model.generate_dmt_har_med_stimulus(N)
    ...
    for t in range(1, N):
        ...
        # Strong nostalgia recording during transition and persistent phases
        if t > 300:
    ...
    fig.suptitle('Simulation 13: DMT-HAR-MED (ds006644)\n'
                 'DensityMA_OS + Higher-Order Emotions (Ultra-Strong Dissolution + Fast Recovery)', fontsize=14, fontweight='bold')

    axs[0].plot(d_eff, color='blue', label='D_eff (DMT + harmine + meditation)')
    ...
    plt.tight_layout()
    plt.savefig('13_DMT_HAR_MED_Simulation_Result.png', dpi=300, bbox_inches='tight')
    print("\n✅ Graph saved as '13_DMT_HAR_MED_Simulation_Result.png'")
    print("Ready for next simulation (14/17).")