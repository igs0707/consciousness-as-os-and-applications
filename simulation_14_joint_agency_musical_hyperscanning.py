...
class DensityMA_OS_Full:
    ...
    Simulation 14: Joint Agency Musical Hyperscanning (ds007471)
    Multiple people playing music simultaneously (collective agency & synchronization)
    """
    def __init__(self):
        # OS Layer
    ...
    def generate_joint_agency_stimulus(self, N=2000):
        """Generate stimulus pattern for Joint Agency Musical Hyperscanning: 
        Collective musical performance with strong interpersonal synchronization"""
        np.random.seed(42)
        d_eff = np.zeros(N)
        
        # Multiple synchronized players (high collective density)
        # Phase 1: Warm-up & entrainment
        d_eff[0:400] = 0.72 + 0.15 * np.random.randn(400)
        
        # Phase 2: Peak collective synchronization (highest Relation_MA scaling)
        d_eff[400:1200] = 0.88 + 0.18 * np.random.randn(800)
        
        # Phase 3: Sustained group flow & resolution
        d_eff[1200:2000] = 0.78 + 0.12 * np.random.randn(800)
        
        # Add group-level rhythmic synchronization waves
        t = np.arange(N)
        collective_rhythm = 0.32 * np.sin(2 * np.pi * t / 95) + 0.18 * np.sin(2 * np.pi * t / 240)
        d_eff += collective_rhythm
        
        # Occasional "group peaks" (full ensemble moments)
        group_peaks = [520, 680, 850, 1020, 1180, 1350, 1520]
        for start in group_peaks:
            if start + 55 < N:
                peak = 0.55 * np.sin(2 * np.pi * np.arange(55) / 14)
                d_eff[start:start+55] += peak
    ...
    def step(self, d_eff, ma_prev, relation_ma_prev, nostalgia_trace, t,
        ...
        # Zero-distance self-embrace (especially strong in collective synchronization)
        embrace_boost = 1.8 if is_shaking else 1.0
        ...
        # Relation_MA update (strong scaling due to collective agency)
        relation_ma_new = np.clip(relation_ma_prev * 0.94 + self.empathy * ma_new * 1.35, 0.0, 3.0)
...
if __name__ == "__main__":
    print("=== Simulation 14: Joint Agency Musical Hyperscanning (ds007471) ===")
    print("Multiple people playing music simultaneously (collective agency & synchronization)")
    print("Testing Relation_MA scaling to collective level through shared self-embrace\n")

    model = DensityMA_OS_Full()
    N = 2000
    d_eff = model.generate_joint_agency_stimulus(N)
    ...
    for t in range(1, N):
        ...
        # Record nostalgia trace during collective peaks
        if t % 130 < 55:
    ...
    print(f"Final Relation_MA: {relation_ma[-1]:.4f}  ← Highest scaling expected")
    ...
    fig.suptitle('Simulation 14: Joint Agency Musical Hyperscanning (ds007471)\n'
                 'DensityMA_OS + Higher-Order Emotions (Collective Musical Synchronization)', fontsize=14, fontweight='bold')

    axs[0].plot(d_eff, color='blue', label='D_eff (Group musical performance)')
    ...
    axs[2].plot(relation_ma, color='orange', label='Relation_MA (Self-Embrace → Collective)')
    ...
    plt.tight_layout()
    plt.savefig('14_Joint_Agency_Musical_Hyperscanning_Simulation_Result.png', dpi=300, bbox_inches='tight')
    print("\n✅ Graph saved as '14_Joint_Agency_Musical_Hyperscanning_Simulation_Result.png'")
    print("Ready for next simulation (15/17).")