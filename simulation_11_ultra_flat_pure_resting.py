...
class DensityMA_OS_Full:
    ...
    Simulation 11: Ultra-Flat Pure Resting
    Extremely flat resting state (D_eff fixed very low)
    """
    def __init__(self):
        # OS Layer
    ...
    def generate_ultra_flat_pure_resting_stimulus(self, N=2000):
        """Generate stimulus pattern for Ultra-Flat Pure Resting: D_eff fixed extremely low (0.20–0.30)"""
        np.random.seed(42)
        d_eff = np.zeros(N)
        
        # Ultra-flat: almost constant very low density
        base = 0.245
        d_eff[:] = base + 0.028 * np.random.randn(N)   # extremely small noise only
        
        # Extremely slow and tiny drift (almost invisible)
        t = np.arange(N)
        ultra_slow_drift = 0.018 * np.sin(2 * np.pi * t / 2400)
        d_eff += ultra_slow_drift
        
        # No significant events → minimal energy, pure negentropy engine test
        return np.clip(d_eff, 0.18, 0.32)
...
if __name__ == "__main__":
    print("=== Simulation 11: Ultra-Flat Pure Resting ===")
    print("Extremely flat resting state (D_eff fixed very low ~0.20–0.30)")
    print("Testing pure negentropy engine and self-embrace with minimal input\n")

    model = DensityMA_OS_Full()
    N = 2000
    d_eff = model.generate_ultra_flat_pure_resting_stimulus(N)
    ...
    for t in range(1, N):
        ...
        # Record nostalgia trace (internal self-repair in ultra-flat state)
        if t % 250 < 90:
    ...
    fig.suptitle('Simulation 11: Ultra-Flat Pure Resting\n'
                 'DensityMA_OS + Higher-Order Emotions (Minimal Input)', fontsize=14, fontweight='bold')

    axs[0].plot(d_eff, color='blue', label='D_eff (Ultra-Flat Pure Resting)')
    ...
    plt.tight_layout()
    plt.savefig('11_Ultra_Flat_Pure_Resting_Simulation_Result.png', dpi=300, bbox_inches='tight')
    print("\n✅ Graph saved as '11_Ultra_Flat_Pure_Resting_Simulation_Result.png'")
    print("Ready for next simulation (12/17).")