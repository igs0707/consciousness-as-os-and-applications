...
class DensityMA_OS_Full:
    ...
    Simulation 05: ds007609 (Resting + Trait Anxiety)
    Resting-state EEG with trait anxiety
    """
    def __init__(self):
        # OS Layer
    ...
    def generate_resting_anxiety_stimulus(self, N=2000):
        """Generate stimulus pattern for ds007609: Resting-state with trait anxiety (rumination fluctuations)"""
        np.random.seed(42)
        d_eff = np.zeros(N)
        
        # Baseline low-density resting state
        d_eff[:] = 0.32 + 0.09 * np.random.randn(N)
        
        # Occasional anxiety-related rumination bursts (internal, non-external)
        anxiety_bursts = [180, 420, 680, 950, 1250, 1580]
        for start in anxiety_bursts:
            if start + 120 < N:
                # Rumination-like slow fluctuation
                t = np.arange(start, start + 120)
                burst = 0.45 * np.sin(2 * np.pi * (t - start) / 85) + 0.12 * np.random.randn(120)
                d_eff[start:start+120] += np.clip(burst, 0.0, 0.6)
        
        # Very gentle overall drift (typical of resting + anxiety)
        drift = 0.05 * np.sin(2 * np.pi * np.arange(N) / 1200)
        d_eff += drift
        
        return np.clip(d_eff, 0.15, 0.78)
...
if __name__ == "__main__":
    print("=== Simulation 05: ds007609 (Resting + Trait Anxiety) ===")
    print("Resting-state EEG with trait anxiety (rumination)")
    print("Testing self-embrace stability during low-input + internal anxiety\n")

    model = DensityMA_OS_Full()
    N = 2000
    d_eff = model.generate_resting_anxiety_stimulus(N)
    ...
    for t in range(1, N):
        ...
        # Record nostalgia trace (self-repair during resting)
        if t % 150 < 50:
    ...
    fig.suptitle('Simulation 05: ds007609 (Resting + Trait Anxiety)\n'
                 'DensityMA_OS + Higher-Order Emotions (Resting with Rumination)', fontsize=14, fontweight='bold')

    axs[0].plot(d_eff, color='blue', label='D_eff (Resting + Trait Anxiety)')
    ...
    plt.tight_layout()
    plt.savefig('05_ds007609_resting_anxiety_Simulation_Result.png', dpi=300, bbox_inches='tight')
    print("\n✅ Graph saved as '05_ds007609_resting_anxiety_Simulation_Result.png'")
    print("Ready for next simulation (6/17).")