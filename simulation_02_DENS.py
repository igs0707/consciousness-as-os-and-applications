...
class DensityMA_OS_Full:
    ...
    Simulation 02: DENS (ds003751)
    Natural video stimuli (daily life and narrative scenes)
    """
    def __init__(self):
        # OS Layer
    ...
    def generate_dens_stimulus(self, N=2000):
        """Generate stimulus pattern for DENS (ds003751): Natural videos with daily life and narrative flow"""
        np.random.seed(42)
        d_eff = np.zeros(N)
        
        # Gentle, sustained natural video pattern (slow emotional accumulation)
        blocks = [
            (0, 500, 0.68),    # Calm daily scene
            (500, 900, 0.75),  # Mild narrative build-up
            (900, 1300, 0.82), # Emotional peak (story development)
            (1300, 1700, 0.71),# Gentle resolution
            (1700, 2000, 0.65) # Return to baseline
        ]
        
        for start, end, base in blocks:
            length = end - start
            # Very smooth noise (low shake)
            noise = 0.08 * np.random.randn(length)
            d_eff[start:end] = base + noise
        
        # Extremely gentle transitions (no sharp switches)
        return np.clip(d_eff, 0.12, 0.92)
...
if __name__ == "__main__":
    print("=== Simulation 02: DENS (ds003751) ===")
    print("Natural video stimuli (daily life and narrative scenes)")
    print("Testing sustained self-embrace under gentle, story-like input\n")

    model = DensityMA_OS_Full()
    N = 2000
    d_eff = model.generate_dens_stimulus(N)
    ...
    for t in range(1, N):
        ...
        # Record nostalgia trace during narrative build-up
        if 400 < t < 1400:
    ...
    fig.suptitle('Simulation 02: DENS (ds003751)\n'
                 'DensityMA_OS + Higher-Order Emotions (Natural Video Stimuli)', fontsize=14, fontweight='bold')

    axs[0].plot(d_eff, color='blue', label='D_eff (Natural daily/narrative videos)')
    ...
    plt.tight_layout()
    plt.savefig('02_DENS_Simulation_Result.png', dpi=300, bbox_inches='tight')
    print("\n✅ Graph saved as '02_DENS_Simulation_Result.png'")
    print("Ready for next simulation (3/17).")