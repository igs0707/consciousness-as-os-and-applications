...
class DensityMA_OS_Full:
    ...
    Simulation 12: Psilocybin (ds006072)
    Psychedelic state (Acute ego dissolution → Persistent re-integration)
    """
    def __init__(self):
        # OS Layer
    ...
    def generate_psilocybin_stimulus(self, N=2000):
        """Generate stimulus pattern for Psilocybin (ds006072): Acute dissolution → Transition → Persistent phase"""
        np.random.seed(42)
        d_eff = np.zeros(N)
        
        # Phase 1: Acute (0-600) - Strong ego dissolution, high chaos
        d_eff[0:600] = 0.75 + 0.32 * np.random.randn(600)
        # Phase 2: Transition (600-1200) - Rapid fluctuations and repair
        d_eff[600:1200] = 0.55 + 0.28 * np.random.randn(600)
        # Phase 3: Persistent (1200-2000) - Deepened, stabilized integration
        d_eff[1200:2000] = 0.48 + 0.15 * np.random.randn(800)
        
        # Add psychedelic-typical gamma-like bursts during Acute phase
        burst_starts = [80, 220, 380, 520]
        for start in burst_starts:
            if start + 45 < N:
                burst = 0.45 * np.sin(2 * np.pi * np.arange(45) / 9)
                d_eff[start:start+45] += burst
        
        # Sharp dissolution drops (ego dissolution moments)
        dissolution_points = [150, 320, 490]
        for p in dissolution_points:
            if p + 30 < N:
                d_eff[p:p+30] *= 0.45   # sudden drop
        
        return np.clip(d_eff, 0.12, 0.98)
    ...
    def step(self, d_eff, ma_prev, relation_ma_prev, nostalgia_trace, t,
        ...
        # Nostalgic repair (stronger in Transition and Persistent phases)
        nostalgia = 0.0
        ...
        # Zero-distance self-embrace (especially strong during dissolution)
        embrace_boost = 2.2 if is_shaking and t < 800 else 1.6 if is_shaking else 1.0
...
if __name__ == "__main__":
    print("=== Simulation 12: Psilocybin (ds006072) ===")
    print("Psychedelic state (Acute ego dissolution → Persistent re-integration)")
    print("Testing extreme dissolution and self-embrace recovery\n")

    model = DensityMA_OS_Full()
    N = 2000
    d_eff = model.generate_psilocybin_stimulus(N)
    ...
    for t in range(1, N):
        ...
        # Record nostalgia trace (especially strong in Transition & Persistent)
        if t > 500:
    ...
    fig.suptitle('Simulation 12: Psilocybin (ds006072)\n'
                 'DensityMA_OS + Higher-Order Emotions (Acute Dissolution → Re-integration)', fontsize=14, fontweight='bold')

    axs[0].plot(d_eff, color='blue', label='D_eff (Psychedelic stimulus)')
    ...
    plt.tight_layout()
    plt.savefig('12_Psilocybin_Simulation_Result.png', dpi=300, bbox_inches='tight')
    print("\n✅ Graph saved as '12_Psilocybin_Simulation_Result.png'")
    print("Ready for next simulation (13/17).")